...
**Optional masked recordings:** Where available, Profile offers an optional recording control. Declining does not affect functionality. Recording starts disabled and requires your opt-in on this device. While you remain opted in, we record each session on that device so we can understand complete journeys, including the steps leading to a problem. Recording begins only after opt-in; earlier activity is not captured. User-entered text and personal content are masked, map and media surfaces are blocked, and network bodies and console recording are disabled. Recordings are retained in PostHog for 30 days. You can turn recordings off in Profile; withdrawal stops further recording.
